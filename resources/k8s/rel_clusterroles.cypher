...
WITH items, items.kind as kind, items.metadata as m

// Get the role node
MATCH (role {name: m.name, uid: m.uid})
WITH role, items, kind, m

// Iterate over each rule
UNWIND items.rules as rules
    //
    WITH rules.resources as resources, rules.verbs as verbs, apoc.convert.toList(rules.resourceNames) as resourceNames, m, kind, role, rules.apiGroups as groups
        ...
        UNWIND verbs as verb
            UNWIND groups as group
                ...
                WITH *, apoc.text.replace(group, '\*', '.*') as group
                    ...
                    WITH role, resource, relname.relname as relname, resourceNames, group
                        CALL apoc.do.case(
                            [
                                // API group and resource names present
                                // Seems as though the apiGroups key is always present so it produces an empty list, but
                                // resourceNames is only present when there are values. So an empty list check is requied groups
                                // and resourceNames has to be compared to null.
                                //
                                // resource is always treated as a regex
                                NOT isEmpty(group) AND NOT group = "" and resourceNames IS NOT NULL,
                                'MATCH (res) WHERE res.kind =~ resource and res.name in resourceNames and res.`spec.group` =~ group RETURN res',

                                // API group present
                                NOT isEmpty(group) AND NOT group = "",
                                'MATCH (res) WHERE res.kind =~ resource and res.`spec.group` =~ group RETURN res',

                                // resourceNames present
                                resourceNames IS NOT NULL,
                                'MATCH (res) WHERE res.kind =~ resource and res.name in resourceNames RETURN res'
                            ],
                            
                            // default
                            'MATCH (res) WHERE res.kind =~ resource RETURN res',
                            {resource: resource, resourceNames: resourceNames, group: group}
                        ) YIELD value as res
